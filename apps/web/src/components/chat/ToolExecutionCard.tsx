...
const TOOL_LABELS: Record<string, string> = {
    ...
    attach_image_to_document: "Attach Image",
    // Image Analysis & Refinement Tools (Feature 023)
    list_document_images: "List Document Images",
    analyze_image: "Analyze Image",
    analyze_document_images: "Analyze Document Images",
...
export default function ToolExecutionCard({
    ...
    const handleCancel = async (e: React.MouseEvent) => {
        ...
        try {
            await api.post("/api/chat/tool-cancel", { tool_call_id: id })
        } catch (error) {
    ...
    const extractImagePreviews = (resultData: unknown): { url: string; title?: string }[] => {
        ...
        if (data.images && Array.isArray(data.images)) {
            data.images.forEach((img: Record<string, unknown>) => {
                if (img.thumbnail_url || img.file_url) {
                    previews.push({
                        url: (img.thumbnail_url || img.file_url) as string,
        ...
        if (data.thumbnail_url) {
            previews.push({
                ...
                title: (data.image_title || data.title) as string | undefined
            })
        }

        // refine_image: result.refined_thumbnail_url
        if (data.refined_thumbnail_url) {
            previews.push({
                url: data.refined_thumbnail_url as string,
                title: 'Refined Image'
            })
        }
        ...
        if (data.analyses && Array.isArray(data.analyses)) {
            data.analyses.forEach((analysis: Record<string, unknown>) => {
                if (analysis.thumbnail_url) {
                    previews.push({
                        url: analysis.thumbnail_url as string,
                        title: (analysis.title || analysis.image_title) as string | undefined
                    })
                }
            })
        }
