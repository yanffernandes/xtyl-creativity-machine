...
/**
 * StylePresetGrid Component
 * Feature 027: Visual Generation Studio
 *
 * Displays a horizontal scrollable grid of style presets with
 * "None" option and category grouping.
 */
...
import type { StylePreset } from '@/types/image-studio';

interface StylePresetGridProps {
  presets: StylePreset[];
  selectedPresetSlug: string | null;
  onSelectPreset: (slug: string | null) => void;
  isLoading?: boolean;
  className?: string;
  title?: string;
...
export function StylePresetGrid({
  ...
  const sortedPresets = [...presets].sort((a, b) => a.sort_order - b.sort_order);
      ...
      <div className={cn('space-y-2', className)}>
        ...
        <div className="flex gap-2 overflow-hidden">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="flex flex-col items-center gap-1.5 p-2 animate-pulse"
            >
    ...
    <div className={cn('space-y-2', className)}>
        ...
        <div className="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-400">
          ...
          {selectedPresetSlug && (
            <span className="text-blue-500">
              ({sortedPresets.find((p) => p.slug === selectedPresetSlug)?.name_pt || selectedPresetSlug})
            </span>
          )}
        </div>
      ...
      <div className="relative">
        <div
          ...
          className={cn(
            'flex gap-1 overflow-x-auto scrollbar-hide pb-2',
        ...
        >
            ...
            <motion.button
              ...
              className={cn(
                ...
                selectedPresetSlug === null &&
                  'bg-blue-500/10 dark:bg-blue-500/20 ring-2 ring-blue-500/50'
              )}
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  selectedPresetSlug === null && 'text-blue-600 dark:text-blue-400'
                )}
              >
                Nenhum
              </span>
          ...
          {sortedPresets.map((preset) => (
            <div key={preset.id} className="flex-shrink-0 snap-start">
              <StylePresetCard
                preset={preset}
                isSelected={selectedPresetSlug === preset.slug}
                onSelect={(p) => onSelectPreset(p.slug)}
              />
            </div>
          ))}
        </div>
