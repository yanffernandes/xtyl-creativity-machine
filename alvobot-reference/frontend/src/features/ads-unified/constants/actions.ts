// ============================================================================
// AUTOMATION ENGINE — ACTION DEFINITIONS (Frontend)
// All available automation actions, organized by platform + level.
...
export const ALL_ACTIONS: ActionDefinition[] = [
  ...
  // ── Naming ──
  {
    ...
    category: 'Naming',
    description: 'Adiciona texto ao nome da entidade (prefixo ou sufixo)',
    requiresParams: true,
  },
  {
...
export const ACTIONS_BY_PLATFORM_LEVEL: Record<string, Record<string, string[]>> = {
  meta: {
    campaign: [
      'pause',
      'start',
...
/**
 * Get actions grouped by category for a given platform + level.
 */
export function getActionsGroupedByCategory(
  platform: string,
  level: string,
): Record<string, ActionDefinition[]> {
  const actions = getActionsForPlatformLevel(platform, level)
...
/**
 * Get the label for an action (or the slug itself as fallback).
 */
export function getActionLabel(slug: string): string {
  return findAction(slug)?.label ?? slug
}
